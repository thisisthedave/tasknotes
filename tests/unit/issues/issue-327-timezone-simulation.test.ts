...
describe('Issue #327: Timezone Date Handling Simulation', () => {
    it('demonstrates potential date mismatch between agenda view and toggle function', () => {
        ...
        console.log('Normalized date formatted:', formatUTCDateForCalendar(normalizedDate));
        ...
        console.log('Reconstructed formatted:', formatUTCDateForCalendar(reconstructedDate));
        ...
        console.log('Formatted:', formatUTCDateForCalendar(dateFromString));
        ...
        expect(formatUTCDateForCalendar(reconstructedDate)).toBe('2024-01-17');
        expect(formatUTCDateForCalendar(dateFromString)).toBe('2024-01-17');
    });
    ...
    it('shows how timezone differences can cause date shifts', () => {
        ...
        console.log('UTC formatted:', formatUTCDateForCalendar(pacificDate));
        
        // The UTC date is Jan 17, but the local date was Jan 16
        expect(formatUTCDateForCalendar(pacificDate)).toBe('2024-01-17');
        ...
        console.log('UTC formatted:', formatUTCDateForCalendar(utcDate));
        
        expect(pacificDate.getTime()).toBe(utcDate.getTime()); // Same moment in time
        expect(formatUTCDateForCalendar(utcDate)).toBe('2024-01-17');
    });
    ...
    it('simulates the exact bug scenario from issue #327', () => {
        ...
        console.log('Formatted for calendar:', formatUTCDateForCalendar(localJuly29));
        
        // The bug occurs when formatUTCDateForCalendar returns a different day
