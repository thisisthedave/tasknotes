...
export class TaskNotesSettingTab extends PluginSettingTab {
	...
	private renderTaskDefaultsTab(): void {
		...
		helpList.createEl('li', { text: '{{parentNote}} - Parent note link formatted as YAML list item (only for instant convert)' });
		
		helpContainer.createEl('p', { 
			text: 'Template is applied when the task is created with all final values from the form. Use {{details}} to include user content from the Details field.\n{{parentNote}} will resolve to a YAML list item with newline and dash prefix (e.g., "\\n- [[Note Name]]") for the note where a checkbox task was converted to a tasknote (empty for manually created tasks). Variables use the same format as daily note templates.',
			cls: 'settings-help-note'
		});
	}
